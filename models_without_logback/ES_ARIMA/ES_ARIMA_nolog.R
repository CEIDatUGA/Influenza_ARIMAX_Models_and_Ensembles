#' ES_ARIMA 
#' This function utilizes ensembles and single automatic ARIMA models.
#' The function fits a rolling windows of N weeks to generate forecasts.
...
#' The user defines if it will use an AUTO ARIMA (auto=TRUE), or ensembles of 27 (ES27=TRUE) or 64 models (ES64=TRUE), chooses the number of weeks ahead for each forecast, and the size of the rolling window.
#' 
#' @param current_state_tb A list containing Influenza Like Illness tibbles for one or many U.S. state.
...
ES_ARIMA<-function(current_state_tb, auto=FALSE, n_weeks_ahead=1, ES27=TRUE, ES64=FALSE, window=104){
  ...
  results[[1]]<- ROLLING_ARIMA(current_state_tb, n_weeks_ahead=n_weeks_ahead, window = window, order_params=my_order_params, auto=auto) 
...
#' ROLLING_ARIMA
#'
#' This function works together with the ES_ARIMA function.
...
ROLLING_ARIMA <- function(current_state_tb, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE) {
  ...
  for(iter in  2:(NROW(current_state_tb$cases)-(window))){ # rolling window of 104 weeks up to the end of the ILI cases dataset
    ...
    fitted_models<-list()
    # Rolling window of 104 weeks.
    rolling_window<- iter:(window+iter-1) 
    # AIC scores for weighting the ensembles
    model_aic_scores<-c() 
    # Each model id, important later for the ensembles
    model_id<-1
...
##########   
    ...
    if(length(current_state_tb$cases[rolling_window])==window){ 
      ...
      for(j in 1:nrow(order_params)){ # run 1 time for auto.arima, of 27 or 64 times for the ensembles.
        ...
        tryCatch(
          expr = {
            ...
            if(!auto){ 
              ...
              fit<-Arima(current_state_tb$cases[rolling_window], order = order_params[j,], method = "CSS-ML") 
              }
            # if auto = TRUE, run auto.arima
            else{
              set.seed(1)
              # fit an auto.arima 
              fit<-invisible(auto.arima(current_state_tb$cases[rolling_window], stepwise=TRUE)) #stepwise is a fast auto.arima process
              }
...
##############
      ...
      for(my_model in fitted_models){
        ...
        if(length(my_model)>0 && !is.na(my_model[1]) && !(is.na(my_model$aic))){ 
          ...
          fc <- forecast(my_model, h=n_weeks_ahead, level=99) ### forecast 
...
#' FormatForWIS
#'
#' This function formats the ROLLING_ARIMA results for calculating the weighted interval score.
#' 
...
#' @param n_weeks_ahead The number of weeks ahead the model is forecasting. This is important for calculating the data in which the forecast was made. I uses the same value as the ROLLING_ARIMA.
#' 
#' @return A list containing the forecast, predictive quantiles and number of models in each ensemble.
#'
