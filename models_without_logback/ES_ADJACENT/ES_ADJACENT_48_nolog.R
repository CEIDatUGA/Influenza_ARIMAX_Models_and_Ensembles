...
ES_ADJACENT<-function(current_state_tb, auto=FALSE, n_weeks_ahead=1, week_lag=1, ES27=TRUE, ES64=FALSE, window=104, list_of_states=list_of_states){
  ...
  results[[1]]<- ROLLING_ARIMAX_ADJACENT(current_state_tb, n_weeks_ahead=n_weeks_ahead, week_lag=week_lag, window = window, order_params=my_order_params, auto=auto, list_of_states=list_of_states) # %packages% "forecast" 
...
#' ROLLING_ARIMAX_ADJACENT
#'
#' This function works together with the ES_ADJACENT function.
...
ROLLING_ARIMAX_ADJACENT <- function(current_state_tb, week_lag=week_lag, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE, list_of_states=list_of_states) {
  ...
  us_adjacent_states <- list(
    Alabama = c(8, 21, 39, 48), # Georgia, Mississippi, Tennessee 
    ...
    Georgia = c(1, 30, 37, 39,48), # Alabama, Florida, North Carolina, South Carolina, Tennessee
        ...
        tryCatch(
          expr = {
            if(!auto){
              ...
              set.seed(1)
              # fit ensembles of ARIMAs of the data
              fit<-Arima(current_state_tb$cases[current_state_rolling_window], xreg=adjacent_states_dataset[,1], order = order_params[j,], method = "CSS-ML") #
            }
            
            # if auto = TRUE, run auto.arima
            else{
              set.seed(1)
              fit<-invisible(auto.arima(current_state_tb$cases[current_state_rolling_window], xreg=adjacent_states_dataset[,1] ,stepwise=TRUE)) # trace my not be avaliable
            }
...
#' FormatForWIS
#'
#' This function formats the ROLLING_ARIMAX_ADJACENT results for calculating the weighted interval score.
#' 
...
#' @param n_weeks_ahead The number of weeks ahead the model is forecasting. This is important for calculating the data in which the forecast was made. I uses the same value as the ROLLING_ARIMAX_ADJACENT.
#' 
#' @return A list containing the forecast, predictive quantiles and number of models in each ensemble.
#'
