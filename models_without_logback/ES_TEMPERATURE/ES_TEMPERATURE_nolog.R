        ...
        tryCatch(
          expr = {
            if(!auto){
              ...
              fit<-Arima(current_state_tb$cases[current_state_rolling_window], xreg=temp_rolling_window[,1], order = order_params[j,], method = "CSS-ML") #
            }
            # if auto = TRUE, run auto.arima
              ...
              # fit an auto.arima 
              # use the temperature data on based on a lag rolling window as exogenous variable 
              fit<-invisible(auto.arima(current_state_tb$cases[current_state_rolling_window], xreg=temp_rolling_window[,1] ,stepwise=TRUE)) # stepwise is a fast auto.arima
            }
