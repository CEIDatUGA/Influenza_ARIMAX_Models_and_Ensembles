        ...
        tryCatch(
          expr = {
            if(!auto){
              ...
              fit<-Arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=log1p(AVERAGE_states_dataset[,1]), order = order_params[j,], method = "CSS-ML") #
            }
            
            # if auto = TRUE, run auto.arima
            else{
              set.seed(1)
              fit<-invisible(auto.arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=log1p(AVERAGE_states_dataset[,1]) ,stepwise=TRUE)) # trace my not be avaliable
            }
