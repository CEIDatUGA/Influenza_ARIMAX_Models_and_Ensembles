        ...
        tryCatch(
          expr = {
            if(!auto){
              ...
              fit<-Arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=log1p(epiweek_dataset[,1]), order = order_params[j,], method = "CSS-ML") #
            }
            # if auto = TRUE, run auto.arima
              ...
              set.seed(1)
              # use the epiweek data on based on a lag rolling window as exogenous variable 
              fit<-invisible(auto.arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=log1p(epiweek_dataset[,1]) ,stepwise=TRUE)) # stepwise is a fast auto arima
            }
...
##############
      ...
      for(my_model in fitted_models){
        ...
        if(length(my_model)>0 && !is.na(my_model[1]) && !(is.na(my_model$aic))){ 
          ...
          fc <- forecast(my_model, h=n_weeks_ahead, xreg=log1p(exog_var[1:n_weeks_ahead]), level=99) # forecast for 99% confidence interval
