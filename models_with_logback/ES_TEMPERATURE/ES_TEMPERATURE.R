#' ES_TEMPERATURE
#' This function utilizes ensembles and single automatic ARIMAX models which have mean temperature as exogenous variables.
...
#' @param temperature_data A data frame containing weekly mean temperature data for ALL U.S. states for the same weeks as the ILI data.
...
ES_TEMPERATURE<-function(current_state_tb, week_lag=1, temperature_data=NULL, auto=FALSE, n_weeks_ahead=1, ES27=TRUE, ES64=FALSE, window=104){
  ...
  results[[1]]<- ROLLING_ARIMAX_TEMPERATURE(current_state_tb, temperature_data=temperature_data, week_lag=week_lag, n_weeks_ahead=n_weeks_ahead, window = window, order_params=my_order_params, auto=auto)  
...
#' ROLLING_ARIMAX_TEMPERATURE
#'
#' This function works together with the ES_TEMPERATURE function.
...
ROLLING_ARIMAX_TEMPERATURE <- function(current_state_tb, week_lag=week_lag, temperature_data=NULL, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE) {
  
  set.seed(1)
  # Number of models in each ensemble by date
  N_of_models<-c() 
  # Empty predictions 
  prediction<-list() 
  # Empty predictions and dates data frame 
  prediction_df<- data.frame("predicted_date"= NULL, "Prediction" = NULL) # empty data frame for forecasts and dates
  # Empty prediction quantiles lists
  ...
  for(iter in (1+week_lag):(NROW(current_state_tb$cases)-(window))){ 
    ...
    # The temp_rolling_window uses state level from the temperature dataset
    # It chooses the column based on the state index number and slice it based on the exog_rolling_window
    temp_rolling_window<-data.frame(temperature_data[,current_state_tb$location[1]][exog_rolling_window]) 
    #########################################
    # Exogenous variable for each iteration #
    # temperature data for the day in which the forecast is being made
    temp_exog_<-data.frame(temperature_data[,current_state_tb$location[1]][current_state_rolling_window])
    exog_var<-c(temp_exog_[104,1], temp_exog_[104,1], temp_exog_[104,1], temp_exog_[104,1])
        ...
        tryCatch(
          expr = {
            if(!auto){
              ...
              # use the temperature data on based on a lag rolling window as exogenous variable 
              fit<-Arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=temp_rolling_window[,1], order = order_params[j,], method = "CSS-ML") #
            }
            # if auto = TRUE, run auto.arima
              ...
              # use the temperature data on based on a lag rolling window as exogenous variable 
              fit<-invisible(auto.arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=temp_rolling_window[,1] ,stepwise=TRUE)) # stepwise is a fast auto.arima
            }
...
##############
      ...
      for(my_model in fitted_models){
        ...
        if(length(my_model)>0 && !is.na(my_model[1]) && !(is.na(my_model$aic))){ 
          ...
          # use the temperature exogenous variable of the current date in which the forecast is being made
          fc <- forecast(my_model, h=n_weeks_ahead, xreg=exog_var[1:n_weeks_ahead], level=99)### forecast for 99% confidence
...
#' FormatForWIS
#'
#' This function formats the ROLLING_ARIMAX_TEMPERATURE results for calculating the weighted interval score.
#' 
...
#' @param n_weeks_ahead The number of weeks ahead the model is forecasting. This is important for calculating the data in which the forecast was made. I uses the same value as the ROLLING_ARIMAX_TEMPERATURE.
#' 
#' @return A list containing the forecast, predictive quantiles and number of models in each ensemble.
#'
